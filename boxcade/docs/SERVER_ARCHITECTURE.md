# Blobcade server architecture — relay + nodes, load model & hosting costs

> Companion to [`tauri.md`](../tauri.md). `tauri.md` covers wrapping the client
> in Tauri (desktop/Android/iOS/browser). This doc covers the **server** that all
> those clients talk to: how it scales, how future "server nodes" attach, how the
> hosted server becomes a **relay/coordinator** for those nodes, the real load
> profile derived from the code, and a cost comparison of **AWS/Azure vs VPS
> hosters vs community-federated** deployment.
>
> Scope: research only — no code changed. Aligned to the existing
> [`ROADMAP.md`](./ROADMAP.md) (Phase 5 authoritative multiplayer, `NET-001/002`,
> `OPS-005`).
>
