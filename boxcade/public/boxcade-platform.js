(function () {
  ...
  window.addEventListener('message', function (event) {
    var data = event.data || {}
    if (!data || data.t !== 'boxcade:hello' || data.v !== VERSION) return
  ...
  window.Boxcade = {
    ...
    awardBolts: function (n, reason) {
      var amount = Math.max(1, Math.min(10, Math.floor(Number(n) || 0)))
      send({ t: 'boxcade:awardBolts', v: VERSION, n: amount, reason: String(reason || '').slice(0, 80) })
    },
    submitScore: function (seconds) {
      var score = Number(seconds)
      if (!Number.isFinite(score) || score <= 0) return
      send({ t: 'boxcade:submitScore', v: VERSION, seconds: score })
    },
  }

  window.parent.postMessage({ t: 'boxcade:ready', v: VERSION }, '*')
})()
