# Blobcade × Tauri — full research & integration plan

> Goal: ship Blobcade as a native app on **desktop (Windows / macOS / Linux),
> Android, iPhone / iPad, and the browser** from one codebase.
>
> This document is the research output: it maps every coupling point in the
> current codebase, the hard constraints of each platform, the realistic
> architecture options, and a concrete, phased plan with file/line references.
>
> Status: **research only** — no code changed. `src-tauri/` does not exist yet.
...
| File | Line | Code |
| --- | --- | --- |
| `src/api.ts` | 7 | `const base = () => `${location.protocol}//${location.hostname}:8081`` |
| `src/engine/network.ts` | 68 | `const url = `ws://${location.hostname}:8081`` |
| `src/portal.ts` | 128 | `const ws = new WebSocket(`ws://${location.hostname}:8081`)` |
...
coordinator + relay design in [`docs/SERVER_ARCHITECTURE.md`](./docs/SERVER_ARCHITECTURE.md)
